/**
 * clean_fys240.js — fork of clean.js with a few extra macro handlers for
 * custom commands that appear in the FYS.240 Optics .tex sources but not in
 * the FYS.501 Laser Physics ones the original pipeline was built against:
 *   - \fraca{a}{b}                              -> (a)/(b)            (like \frac)
 *   - \Underlinecomment{color}{content}{note}   -> content            (drop color+note)
 *   - \Underlineshortcomment{color}{content}{note} -> content         (drop color+note)
 *   - \textcolor{color}{content}                -> content            (drop color)
 *   - \colorlet{...}{...}, \setlength{...}{...}, \addtocounter{...}{...} -> dropped
 *   - \bg{...} / \BIg... sizing variants        -> treated like \big/\Big
 * Everything else is identical to clean.js.
 *
...
// Unwraps a three-argument macro \name{arg1}{arg2}{arg3}, keeping only the
// argument at position `keep` (1, 2 or 3). Used for custom FYS.240 macros
// like \Underlinecomment{color}{content}{note} where only the middle
// (content) argument is corpus-worthy.
function unwrapThreeArgMacro(text, name, keep = 2) {
  const re = new RegExp(`\\\\${name}\\{`);
  let out = text;
  let guard = 0;
  while (guard++ < 5000) {
    ...
    if (close1 === -1) { out = out.slice(0, m.index) + out.slice(m.index + m[0].length); continue; }
    const inner1 = out.slice(open1 + 1, close1);
    const after1 = out.slice(close1 + 1);
    if (!/^\{/.test(after1)) {
      out = out.slice(0, m.index) + (keep === 1 ? inner1 : '') + out.slice(close1 + 1);
      continue;
    }
    const open2 = close1 + 1;
    const close2 = matchBrace(out, open2);
    if (close2 === -1) { out = out.slice(0, m.index) + out.slice(open2 + 1); continue; }
    const inner2 = out.slice(open2 + 1, close2);
    const after2 = out.slice(close2 + 1);
    if (!/^\{/.test(after2)) {
      out = out.slice(0, m.index) + (keep === 1 ? inner1 : keep === 2 ? inner2 : '') + out.slice(close2 + 1);
      continue;
    }
    const open3 = close2 + 1;
    const close3 = matchBrace(out, open3);
    if (close3 === -1) { out = out.slice(0, m.index) + out.slice(open3 + 1); continue; }
    const inner3 = out.slice(open3 + 1, close3);
    const kept = keep === 1 ? inner1 : keep === 2 ? inner2 : inner3;
    out = out.slice(0, m.index) + kept + out.slice(close3 + 1);
  }
  return out;
}

// Drops a whole \name{a}{b} two-argument construct (both arguments), e.g.
// \colorlet{alias}{basecolor} or \setlength{\dim}{2cm} — neither argument is
// corpus-worthy text.
function dropTwoArgMacro(text, name) {
  const re = new RegExp(`\\\\${name}\\{`);
  let out = text;
  let guard = 0;
  while (guard++ < 5000) {
    ...
    if (close1 === -1) { out = out.slice(0, m.index) + out.slice(m.index + m[0].length); continue; }
    const after1 = out.slice(close1 + 1);
    if (!/^\{/.test(after1)) { out = out.slice(0, m.index) + out.slice(close1 + 1); continue; }
    const open2 = close1 + 1;
    const close2 = matchBrace(out, open2);
    if (close2 === -1) { out = out.slice(0, m.index) + out.slice(open2 + 1); continue; }
    out = out.slice(0, m.index) + out.slice(close2 + 1);
  }
  return out;
}
...
function dropStructuralMacros(text) {
  ...
  for (const name of ['colorlet', 'setlength', 'addtocounter']) {
    out = dropTwoArgMacro(out, name);
  }
...
function convertMathMacros(text) {
  ...
  // \frac{a}{b} / \Frac{a}{b} / \fraca{a}{b} (custom FYS.240 fraction macro,
  // seemingly a variant spacing/sizing of \frac) -> (a)/(b), innermost-first
  const fracRe = /\\(?:[Ff]rac|fraca|dfrac|tfrac)\{([^{}]*)\}\{([^{}]*)\}/;
  let guard = 0;
  ...
  // \textcolor{color}{content} -> content; custom underline+comment macros
  // \Underlinecomment{color}{content}{note} / \Underlineshortcomment{...}
  // -> content only (drop the color arg and the side-note arg)
  out = unwrapTwoArgMacro(out, 'textcolor', 2);
  out = unwrapThreeArgMacro(out, 'Underlinecomment', 2);
  out = unwrapThreeArgMacro(out, 'Underlineshortcomment', 2);
  ...
  // always grabs the macro's full name (no partial-prefix collisions). Also
  // consumes an immediately-following [optional] group when the macro DID
  // resolve to a symbol (e.g. custom bracket macros like \LP[4]{...} used
  // for sizing) — but only then, so real bracketed math like "[1+a]^{1/2}"
  // elsewhere in the text is left untouched.
  out = out.replace(/\\([A-Za-z]+)(\[[^\]]*\])?/g, (m, name, optArg) => {
    const rep = GREEK[name] ?? OPERATOR_WORDS[name];
  ...
  out = out.replace(
    /\\(?:BIGGL|BIGGR|Biggl|Biggr|BIGG|Bigg|biggl|biggr|bigg|BIGL|BIGR|Bigl|Bigr|BIG|BIg|Big|big|bg|bigc|left|right)(?:\[[^\]]*\])?\s*([|()[\]{}.]?)/g,
    (_, d) => (d && d !== '.' ? d : '')
  );
...
function convertHighlightMacros(text, { mark = false } = {}) {
  ...
  // CUAlert / UAlert — colored/underlined variants of Alert seen in a few
  // FYS.240 slides; same treatment (they mark terms the same way).
  out = unwrapMacro(out, 'CUAlert', { transform: wrap });
  out = unwrapMacro(out, 'UAlert', { transform: wrap });
  out = unwrapMacro(out, 'culine'); // underline styling only, not a term highlight
  // DUTline / CUTline — more underline-styling variants (not term highlights)
  out = unwrapMacro(out, 'DUTline');
  out = unwrapMacro(out, 'CUTline');
  return out;
}
...
function convertMisc(text) {
  ...
  // A few FYS.240 slides pass a bare reference URL as a bogus extra brace
  // argument (e.g. \CDAlert[blue]{linear polarization}{https://en.wikipedia...}
  // instead of using \href) — drop any standalone {https://...}/{http://...}
  // group outright; it's a citation, never display text.
  ...
  // \fadingtext[opts]{tikz-style-string}{content} -> content (2nd arg);
  // the first arg is a raw tikz "path picture shading=..." style string,
  // never display text. Occurs once in the FYS.240 slides (section 2.1).
  out = unwrapTwoArgMacro(out, 'fadingtext', 2);
  // \Dashuline{content} -> content (dashed-underline styling only)
  out = unwrapMacro(out, 'Dashuline');
  ...
  // \href{url}{text} -> text  (drop the URL, keep the visible text; occurs
  // in a few FYS.240 slides linking out to Wikipedia articles)
  ...
  // \makebox[width][pos]{content} — collapse its (up to two) bracket args
  // down to nothing before the generic handler runs, so only the content
  // survives.
  out = out.replace(/\\makebox(\[[^\]]*\])(\[[^\]]*\])?/g, '\\makebox');

  // \begin{minipage}[opt]{width} ... \end{minipage} -> keep content,
...
function convertMathEnvAndDelimiters(text) {
  ...
  // \\[<length>] — a line break with extra vertical spacing (e.g. \\[-2.5mm])
  // — MUST run before the single \[ / \] display-math rule just below, or
  // that rule consumes the second backslash + bracket first (matching just
  // "\[" of the pair) and orphans the first backslash plus the leaked
  // "-2.5mm]" text.
  out = out.replace(/\\\\\[[^\]]*\]/g, '\n');
  out = out.replace(/\\\[/g, '\n').replace(/\\\]/g, '\n');
  out = out.replace(/\$\$/g, '\n');
...
function catchAll(text) {
  ...
  // safety net: any remaining lone backslash (not part of a word-macro,
  // e.g. an orphaned "\" from an edge case elsewhere in the pipeline) —
  // never meaningful content, so it's dropped rather than left visible.
  out = out.replace(/\\/g, '');
...
module.exports = {
  matchBrace, unwrapMacro, unwrapTwoArgMacro, unwrapThreeArgMacro, dropTwoArgMacro, removeEnvironment, unwrapEnvironment,
